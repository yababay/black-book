...
(async () => {
    ...
    const keys = await client.keys('black-book:topic:memo:*')
    let ok = []
    ...
    for(const rec of ok) {
        const { id } = rec
        await client.rPush('black-book:queue:memo', id)
    }
        /*let ok = []
    for(const id of arr) {
    ...
    console.log('there are', keys.length, ok.length)
    //await client.save()
    await client.quit()
})()
